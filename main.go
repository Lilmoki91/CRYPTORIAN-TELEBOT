...
// --- KONSTAN AUDIO ---
// [FIXED] - Buang ruang putih ekstra di hujung URL
			...
			if len(mediaGroup) > 0 {
				...
				if resp, err := bot.Request(album); err == nil {
					...
					if err := json.Unmarshal(resp.Result, &sentMessages); err == nil {
						...
						// [PENAMBAHAN] Fallback jika unmarshal gagal
						// Hantar teks dan gambar-gambar individu
						msg := tgbotapi.NewMessage(chatID, caption.String())
						msg.ParseMode = tgbotapi.ModeMarkdownV2
						if sentMsg, err := bot.Send(msg); err == nil {
							(*messageIDs)[chatID] = append((*messageIDs)[chatID], sentMsg.MessageID)
						}
						for _, imgURL := range step.Images {
							photo := tgbotapi.NewPhoto(chatID, tgbotapi.FileURL(imgURL))
							if sentPhoto, err := bot.Send(photo); err == nil {
								(*messageIDs)[chatID] = append((*messageIDs)[chatID], sentPhoto.MessageID)
							} else {
								log.Printf("Fallback: Gagal menghantar photo individu: %v", err)
							}
						}
					}
				} else {
					log.Printf("Gagal menghantar MediaGroup: %v", err)
					// [PENAMBAHAN] Fallback jika request album gagal
					// Hantar teks dan gambar-gambar individu
					msg := tgbotapi.NewMessage(chatID, caption.String())
					msg.ParseMode = tgbotapi.ModeMarkdownV2
					if sentMsg, err := bot.Send(msg); err == nil {
...
func main() {
	...
	for update := range updates {
		if update.CallbackQuery != nil {
		 // [FIXED] - Semak jika Message dalam CallbackQuery adalah nil
		 if update.CallbackQuery.Message == nil {
             log.Printf("Mesej CallbackQuery adalah nil, menghentikan pemrosesan untuk ID: %s", update.CallbackQuery.ID)
             // Masih penting untuk mengakui callback query walaupun tiada mesej
             bot.Request(tgbotapi.NewCallback(update.CallbackQuery.ID, ""))
             continue // Lewatkan proses lanjut
		 }
			...
			case "get_guide_claim":
				...
				if err := json.Unmarshal(guides["worldcoin_registration_guide"], &guideData); err == nil {
					...
					bot.Send(tgbotapi.NewMessage(chatID, "🚫 Gagal memuatkan panduan Worldcoin\\. Sila cuba lagi\\.")) // Gunakan bot.Send untuk callback
				}
			case "get_guide_wallet":
				...
				if err := json.Unmarshal(guides["hata_setup_guide"], &guideData); err == nil {
					...
					bot.Send(tgbotapi.NewMessage(chatID, "🚫 Gagal memuatkan panduan Wallet HATA\\. Sila cuba lagi\\.")) // Gunakan bot.Send untuk callback
				}
			case "get_guide_cashout":
				...
				if err := json.Unmarshal(guides["cashout_guide"], &guideData); err == nil {
					...
					bot.Send(tgbotapi.NewMessage(chatID, "🚫 Gagal memuatkan panduan Cashout\\. Sila cuba lagi\\.")) // Gunakan bot.Send untuk callback
				}
			}
			continue
		}
		...
		if update.Message != nil && update.Message.Text != "" {
			...
			case "📊 Infografik":
				...
				if err := json.Unmarshal(guides["infographic_guide"], &infographicData); err == nil {
					...
					bot.Send(tgbotapi.NewMessage(chatID, "🚫 Gagal memuatkan data infografik\\. Sila cuba lagi\\.")) // Gunakan bot.Send untuk callback
				}
			case "♻️ Reset Mesej":
