...
func sendDetailedGuide(bot *tgbotapi.BotAPI, chatID int64, guide Guide) {
	...
	if _, err := bot.Send(titleMsg); err != nil {
	...
	for _, step := range guide.Steps {
		...
		if len(step.Images) == 0 {
			...
			if _, err := bot.Send(album); err != nil {
				log.Printf("Failed to send guide step (album) to chat %d: %v", chatID, err)
			}
		}
	}
...
func main() {
	...
	if err != nil {
		// IMPROVEMENT: Use Fatalf for cleaner exit logs
	...
	for update := range updates {
		...
		// FIX: Removed Markdown from case statements to match button text literally
