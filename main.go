...
// FIX: Function now accepts the messageIDs map to store the IDs of messages it sends
	...
	for _, step := range guide.Steps {
		...
		if len(step.Images) == 0 {
			...
			for i := 0; i < totalImages; i += chunkSize {
				...
				// For albums, the API returns an array of messages
...
func main() {
	...
	for update := range updates {
		...
		// FIX: Pass the messageIDsToDelete map to the function
		case "/claim":
		...
		case "/reset mesej", "🔄 Reset (Padam Mesej)":
			...
			msg.ReplyMarkup = initialKeyboard
			if sentMsg, err := bot.Send(msg); err == nil {
                // Also add the confirmation message to the (new) delete list
				messageIDsToDelete[chatID] = append(messageIDsToDelete[chatID], sentMsg.MessageID)
			} else {
