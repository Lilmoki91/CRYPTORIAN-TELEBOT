...
func sendDetailedGuide(bot *tgbotapi.BotAPI, chatID int64, guide Guide, messageIDs *map[int64][]int, mu *sync.Mutex) {
    ...
    if sentMsg, err := bot.Send(titleMsg); err == nil {
        ...
        log.Printf("ERROR hantar tajuk: %v", err)
    }
    ...
    for _, step := range guide.Steps {
        ...
        if sentMsg, err := bot.Send(textMsg); err == nil {
            addMessageID(messageIDs, mu, chatID, sentMsg.MessageID)
        } else {
            log.Printf("ERROR hantar teks step: %v", err)
        }

        // 2. Hantar GAMBAR satu per satu (tanpa kapsyen Markdown kompleks)
        //    Markdown dalam kapsyen gambar juga berfungsi untuk gambar tunggal!
        for i, imgURL := range step.Images {
            photo := tgbotapi.NewPhoto(chatID, tgbotapi.FileURL(imgURL))
            
            // Untuk gambar pertama, kita boleh letak kapsyen ringkas
            if i == 0 && len(step.Images) > 1 {
                photo.Caption = fmt.Sprintf("📸 Gambar %d dari %d", i+1, len(step.Images))
            }
            
            if sentMsg, err := bot.Send(photo); err == nil {
                addMessageID(messageIDs, mu, chatID, sentMsg.MessageID)
            } else {
                log.Printf("ERROR hantar gambar: %v", err)
            }
        }
    }
    ...
    if len(guide.Important.Notes) > 0 {
        ...
        if sentMsg, err := bot.Send(msg); err == nil {
            ...
            log.Printf("ERROR hantar nota: %v", err)
        }
    }
}
...
func sendInfographicGuide(bot *tgbotapi.BotAPI, chatID int64, guide InfographicGuide, messageIDs *map[int64][]int, mu *sync.Mutex) {
    ...
    if sentMsg, err := bot.Send(titleMsg); err == nil {
        ...
        log.Printf("ERROR hantar tajuk infografik: %v", err)
    }
    ...
    if guide.ImageMain != "" {
        ...
        if sentMsg, err := bot.Send(photo); err == nil {
            ...
            log.Printf("ERROR hantar gambar utama: %v", err)
        }
    }
    ...
    for _, step := range guide.Steps {
        ...
        photo.ParseMode = tgbotapi.ModeMarkdown // Markdown BERFUNGSI untuk gambar tunggal!
        if sentMsg, err := bot.Send(photo); err == nil {
            addMessageID(messageIDs, mu, chatID, sentMsg.MessageID)
        } else {
            log.Printf("ERROR hantar step infografik: %v", err)
        }
    }
}
