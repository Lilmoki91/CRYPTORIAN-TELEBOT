...
func sendDetailedGuide(bot *tgbotapi.BotAPI, chatID int64, guide Guide, messageIDsToDelete map[int64][]int) {
	// Send main title
	...
	for update := range updates {
		...
		case "/reset mesej", "🔄 Reset (Padam Mesej)":
			...
			for _, id := range messageIDsToDelete[chatID] {
				...
				if err != nil {
					log.Printf("Gagal padam mesej %d: %v", id, err)
				}
			}
			// Kosongkan slice
			messageIDsToDelete[chatID] = nil

			// Hantar mesej permulaan baru
