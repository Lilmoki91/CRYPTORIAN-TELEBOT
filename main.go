    ...
    for update := range updates {
        ...
        if update.CallbackQuery != nil {
            ...
            if update.CallbackQuery.Data == "show_main_menu" {
                ...
                text := "💡 Sila guna *_Butang Aksi Pantas_* di bawah untuk pautan pantas, atau guna arahan seperti `/claim` untuk panduan langkah demi langkah\\."
        ...
        case "/start", "🔙 Kembali Menu Utama":
            msg := tgbotapi.NewMessage(chatID, "👋 Selamat Datang ke 🤖 *_CRYPTORIAN TELEBOT_*\\! Tekan butang di bawah untuk memaparkan menu\\.")
            msg.ParseMode = tgbotapi.ModeMarkdownV2
        ...
        default:
            msg := tgbotapi.NewMessage(chatID, "❌ Arahan tidak dikenali\\. Sila guna butang atau arahan yang sah seperti    /calim /wallet /cashout\\.")
            sentMsg, _ := bot.Send(msg)
            messageIDsToDelete[chatID] = append(messageIDsToDelete[chatID], sentMsg.MessageID)
