...
func main() {
    ...
    for update := range updates {
        ...
        case "/start", "🔙 Kembali Menu Utama":
            msg := tgbotapi.NewMessage(chatID, "👋 Selamat Datang ke 🤖 CRYPTORIAN TELEBOT\\! Tekan butang di bawah untuk memaparkan menu\\.")
            msg.ParseMode = tgbotapi.ModeMarkdownV2
        ...
        case "👨‍💻 Admin":
            text := "🙏Perlukan bantuan lanjut\\? 📞 Hubungi admin secara terus\\ 🕣 waktu urusan setiap hari\\.\n\n🔗 [Hubungi Admin](https://t.me/johansetia)"
        ...
        default:
            msg := tgbotapi.NewMessage(chatID, "❌ Arahan tidak dikenali\\. Sila guna butang atau arahan yang sah,`/claim`, `/wallet`, `/cashout`\\.")
            sentMsg, _ := bot.Send(msg)
            messageIDsToDelete[chatID] = append(messageIDsToDelete[chatID], sentMsg.MessageID)
