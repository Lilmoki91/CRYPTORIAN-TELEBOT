...
// Backslash (\) juga perlu di-escape.
func escapeMarkdownV2(text string) string {
	// Urutan escape adalah penting untuk mengelakkan konflik.
	// 1. Escape backslash itself (\) - Mesti dilakukan pertama
	text = strings.ReplaceAll(text, `\`, `\\`)
	// 2. Escape underscore (_)
	text = strings.ReplaceAll(text, `_`, `\_`)
	// 3. Escape asterisk (*)
	text = strings.ReplaceAll(text, `*`, `\*`)
	// 4. Escape left square bracket ([)
	text = strings.ReplaceAll(text, `[`, `\[`)
	// 5. Escape right square bracket (])
	text = strings.ReplaceAll(text, `]`, `\]`)
	// 6. Escape left parenthesis (()
	text = strings.ReplaceAll(text, `(`, `\(`)
	// 7. Escape right parenthesis ())
	text = strings.ReplaceAll(text, `)`, `\)`)
	// 8. Escape tilde (~)
	text = strings.ReplaceAll(text, `~`, `\~`)
	// 9. Escape backtick (`)
	// text = strings.ReplaceAll(text, "`", "\\`") // Biasanya tidak perlu jika tidak guna code block
	// 10. Escape greater than (>) - Lebih selamat untuk HTML/XML
	text = strings.ReplaceAll(text, `>`, `\>`)
	// 11. Escape hash (#)
	text = strings.ReplaceAll(text, `#`, `\#`)
	// 12. Escape plus (+)
	...
	// 15. Escape pipe (|)
	text = strings.ReplaceAll(text, `|`, `\|`)
	// 16. Escape left curly brace ({)
	text = strings.ReplaceAll(text, `{`, `\{`)
	// 17. Escape right curly brace (})
	text = strings.ReplaceAll(text, `}`, `\}`)
	// 18. Escape dot (.)
	text = strings.ReplaceAll(text, `.`, `\.`)
	// 19. Escape exclamation mark (!)
	text = strings.ReplaceAll(text, `!`, `\!`)
...
func createInlineKeyboard() tgbotapi.InlineKeyboardMarkup {
	return tgbotapi.NewInlineKeyboardMarkup(
		...
		tgbotapi.NewInlineKeyboardRow(
			// URL DIBERSIHKAN: Ruang tambahan dibuang
...
func sendDetailedGuide(bot *tgbotapi.BotAPI, chatID int64, guide Guide) {
	...
	for _, step := range guide.Steps {
		...
		if len(step.Images) == 0 {
			...
			// Hantar album
			// Dalam v5, bot.Send(album) mengembalikan Message (yg pertama) dan error.
			// Untuk mendapat ID semua mesej dalam album adalah kompleks.
			// Penyelesaian: Simpan ID mesej pertama sahaja untuk fungsi padam.
			sentMsg, err := bot.Send(album)
			if err == nil {
				// Hanya ID mesej pertama dalam album yang dapat disimpan dengan cara ini
				messageIDsToDelete[chatID] = append(messageIDsToDelete[chatID], sentMsg.MessageID)
			} else {
...
func main() {
	...
	for update := range updates {
		...
		if update.CallbackQuery != nil {
			...
			// ✅ DIBAIK PULIH: Menangkap kedua-dua nilai return
			...
			if update.CallbackQuery.Data == "show_main_menu" {
				...
				// String panjang disusun dengan betul
				// URL dibersihkan
		...
		if strings.Contains(lowerText, "http") ||
			...
			// ✅ DIBAIK PULIH: Logik keselamatan yang lebih ketat
			// Cari domain utama
			hasSafeDomain := strings.Contains(lowerText, "worldcoin.org") ||
				strings.Contains(lowerText, "hata.io")

			// Jika ia adalah subdomain, maka tidak selamat
			if strings.Contains(lowerText, ".worldcoin.org") || strings.Contains(lowerText, ".hata.io") {
				hasSafeDomain = false
			}
		...
		case "/start", "🔙 Menu Utama":
			// ✅ DIBAIK PULIH: Escape karakter '!' secara manual
			// Perhatikan \! di akhir baris pertama
		...
		case "/claim":
			...
			if ok {
				...
				bot.Send(msg) // Abaikan error untuk mesej ini
			}
		...
		case "/reset", "🔄 Reset":
			...
			for _, id := range messageIDsToDelete[chatID] {
				...
				if _, err := bot.Request(deleteMsg); err == nil {
					...
					// log.Printf("Error deleting message %d: %v", id, err) // Opsyenal
				}
			}
