...
func escapeMarkdownV2(text string) string {
	...
	text = strings.ReplaceAll(text, `~`, `\~`)
	// 9. Escape backtick (`)
	// text = strings.ReplaceAll(text, "`", "\\`") // Biasanya tidak perlu jika tidak guna code
...
func main() {
	...
	for update := range updates {
		...
		if update.CallbackQuery != nil {
			...
			// Betulkan: bot.Request mengembalikan 2 nilai (APIResponse, error)
			...
			if update.CallbackQuery.Data == "show_main_menu" {
				...
				// Padam butang asal
				// Betulkan: bot.Request mengembalikan 2 nilai
