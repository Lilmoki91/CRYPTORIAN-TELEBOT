...
func main() {
    ...
    for update := range updates {
        ...
        // 2. SISTEM BLACKLIST (SEKATAN MUTLAK)
        if IsBanned(userID) {
            continue // Bot diam, jangan respon apa-apa
        }
        ...
        if update.CallbackQuery != nil {
            ...
            // B. Tidak Setuju (STRICT MODE: KELUAR!)
            ...
            // C. Menu Navigasi (Hanya untuk User Sah / Admin)
        ...
        // 6. GATEKEEPER (T&C CHECK)
        // Admin Bypass ATAU User dah setuju
        isAllowed := IsAdmin(userID) || HasAgreed(userID)

        if !isAllowed && update.Message.Command() != "start" {
            // Halang user yang cuba main tekan command lain sebelum setuju
        ...
        case "/start", "🔙 Kembali Menu Utama":
            if isAllowed {
                // User Sah / Admin: Masuk Menu Utama
