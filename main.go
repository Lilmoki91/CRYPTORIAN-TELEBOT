	...
	for _, step := range guide.Steps {
		...
		if len(step.Images) == 0 {
			...
			for i := 0; i < totalImages; i += chunkSize {
				...
				// FIX: Removed the incorrect for...range loop.
				// bot.Send(album) returns a single message, not a slice of messages.
				if sentMsg, err := bot.Send(album); err == nil {
					// We can only save the ID of the first message in the album.
					(*messageIDs)[chatID] = append((*messageIDs)[chatID], sentMsg.MessageID)
				} else {
