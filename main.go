...
// --- FUNGSI BANTUAN ---
// NOTA: Fungsi escapeMarkdown telah DIBUANG.
// Kita guna ModeMarkdown biasa yang tidak perlu escape titik atau sempang.
...
func sendDetailedGuide(bot *tgbotapi.BotAPI, chatID int64, guide Guide, messageIDs *map[int64][]int, mu *sync.Mutex) {
	...
	for _, step := range guide.Steps {
		...
		caption.WriteString(step.Desc) // Terus masukkan deskripsi
...
func sendInfographicGuide(bot *tgbotapi.BotAPI, chatID int64, guide InfographicGuide, messageIDs *map[int64][]int, mu *sync.Mutex) {
	...
	for _, step := range guide.Steps {
		...
		photo.Caption = caption.String()
		// PENTING: Tukar ke ModeMarkdown (Biasa), jangan guna V2
		photo.ParseMode = tgbotapi.ModeMarkdown
		if sentMsg, err := bot.Send(photo); err == nil {
			addMessageID(messageIDs, mu, chatID, sentMsg.MessageID)
		}
...
func main() {
	...
	for update := range updates {
		...
		// 2. SISTEM BLACKLIST
		// Memanggil fungsi dari terms.go
		if IsBanned(userID) {
			continue
		}
		...
		case "/start", "🔙 Kembali Menu Utama":
			if isAllowed {
				...
				// User Baru -> Tunjuk Terms UI dari terms.go
				...
				// PENTING: BuildTermsUI pulangkan markdown biasa
				msg.ParseMode = tgbotapi.ModeMarkdown
