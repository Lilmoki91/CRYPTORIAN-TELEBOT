...
// FUNGSI DIPERBAIKI: Cara yang lebih kukuh untuk menghantar dan merekod ID
func sendAndTrack(bot *tgbotapi.BotAPI, chatID int64, config tgbotapi.Chattable) {
	// Semak jika ia adalah album gambar (MediaGroup)
	if albumConfig, ok := config.(tgbotapi.MediaGroupConfig); ok {
		if sentMessages, err := bot.Send(albumConfig); err == nil {
	...
	// Untuk semua jenis mesej lain (teks, gambar tunggal)
	if sentMsg, err := bot.Send(config); err == nil {
...
// FUNGSI INI KINI AKAN BERFUNGSI DENGAN sendAndTrack YANG BARU
...
func main() {
	...
	for update := range updates {
		...
		case "🚀 Claim WorldCoin", "💰 Wallet HATA", "💸 Cashout ke Bank", "📢 Channel", "👨‍💻 Admin":
			// Bahagian ini sengaja dipermudahkan untuk menepati logik asal anda
			var text string
			switch update.Message.Text {
		...
		case "/reset mesej", "🔄 Reset (Padam Mesej)":
			if ids, ok := messageIDsToDelete[chatID]; ok && len(ids) > 0 {
				// Cara paling serasi untuk memadam mesej adalah satu persatu
				for _, id := range ids {
