    ...
    for update := range updates {
        ...
        if update.CallbackQuery != nil {
            ...
            if update.CallbackQuery.Data == "show_main_menu" {
                ...
                text := "💡 Sila guna *_Butang Aksi Pantas_* di bawah untuk pautan pantas, atau taip teks arahan seperti `/claim` `/wallet` `/cashout` untuk panduan langkah demi langkah\\."
       ...
       // --- padam mesej ---
        ...
        case "/start", "🔙 Kembali Menu Utama":
            msg := tgbotapi.NewMessage(chatID, "👋 Selamat Datang ke 🤖 *_CRYPTORIAN TELEBOT_*\\❗. Tekan butang di bawah untuk memaparkan menu\\. ✅ Taip teks`/command` untuk arahan\\. `/clam` - urusan WorldApps  `/wallet` - urusan Hata Wallet `/cashout` - urusan withdraw online bank\\.")
            msg.ParseMode = tgbotapi.ModeMarkdownV2
