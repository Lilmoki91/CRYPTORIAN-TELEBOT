...
func main() {
    ...
    // === PENTING UNTUK GOOGLE CLOUD RUN ===
    // Telegram Cloud Run perlu guna webhook, bukan polling.
    port := os.Getenv("PORT")
    if port == "" {
        port = "8080"
    }
    // Set up webhook endpoint di sini jika ingin guna Cloud Run!
    // Untuk reference, tukar polling kepada webhook handler:
    // http.HandleFunc("/webhook", func(w http.ResponseWriter, r *http.Request) {
    //     // Parse update dan handle seperti dalam loop bawah
    // })
    // log.Fatal(http.ListenAndServe(":"+port, nil))

    // POLLING (tidak sesuai untuk Cloud Run, hanya untuk local)
