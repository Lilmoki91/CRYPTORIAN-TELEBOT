...
// debugMarkdownFileSearch logs the current working directory and file stat for markdown.json
func debugMarkdownFileSearch() {
	wd, err := os.Getwd()
	if err != nil {
		log.Printf("Gagal dapatkan working dir: %v", err)
	} else {
		log.Printf("Working dir semasa: %s", wd)
	}

	info, err := os.Stat("markdown.json")
	if err != nil {
		log.Printf("markdown.json tidak ditemui: %v", err)
	} else {
		log.Printf("markdown.json ditemui: size=%d mode=%v modTime=%v", info.Size(), info.Mode(), info.ModTime())
	}
}

// sendDetailedGuide menghantar panduan berstructured langkah demi langkah
...
func main() {
	...
	// Debug where the process is looking for markdown.json
	debugMarkdownFileSearch()
		...
		if update.Message != nil && update.Message.Text != "" {
			...
			case "/start", "🔙 Kembali Menu Utama":
				...
				if sentAudio, err := bot.Send(audio); err == nil {
					...
					errorMsgObj := tgbotapi.NewMessage(chatID, errMsg)
					errorMsgObj.ParseMode = tgbotapi.ModeMarkdownV2
					if sentError, err := bot.Send(errorMsgObj); err == nil {
