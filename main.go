...
func sendAndTrack(bot *tgbotapi.BotAPI, chatID int64, config tgbotapi.Chattable) {
	...
	// Kita periksa jenis 'config' (input) untuk tahu jenis output
	switch conf := config.(type) {
	case tgbotapi.MediaGroupConfig:
		sentMessages, err = bot.SendMediaGroup(conf)
	default:
		var sentMsg tgbotapi.Message
		sentMsg, err = bot.Send(config)
		if err == nil {
			sentMessages = []tgbotapi.Message{sentMsg}
		}
	}
