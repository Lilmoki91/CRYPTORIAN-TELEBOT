...
// FUNGSI DIPERBAIKI: Cara yang betul untuk menghantar & merekod ID
func sendAndTrack(bot *tgbotapi.BotAPI, chatID int64, config tgbotapi.Chattable) {
	...
	// Semak jika 'sent' adalah satu mesej atau beberapa mesej (dari album)
	if messages, ok := sent.([]tgbotapi.Message); ok {
		// Jika ia adalah album, 'sent' adalah satu senarai mesej
		for _, msg := range messages {
			messageIDsToDelete[chatID] = append(messageIDsToDelete[chatID], msg.MessageID)
		}
	} else if message, ok := sent.(tgbotapi.Message); ok {
		// Jika ia adalah mesej tunggal
		messageIDsToDelete[chatID] = append(messageIDsToDelete[chatID], message.MessageID)
	}
}
