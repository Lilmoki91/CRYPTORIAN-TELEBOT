...
func TestEscapeMarkdownV2(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    "_{}[]()~>#\+\-=|.!\\", // Tambah backslash
			expected: `\_`\{`\}`\[`\]`\(`\)`\~`\>`\#`\+`\-`\=`\|`\.`\!`\\`, // Jangkaan berdasarkan fungsi baru
		},
		{
...
func TestCreateInlineKeyboard(t *testing.T) {
	...
	secondRowButtons := []struct {
		...
		{"📢 Channel", "https://t.me/cucikripto"}, // URL dibersihkan
		{"👨‍💻 Admin", "https://t.me/johansetia"},  // URL dibersihkan
	}
...
func TestSecurityCheck(t *testing.T) {
	...
	tests := []struct {
		...
		{"Safe WorldCoin link", "Visit https://worldcoin.org", false}, // Dianggap selamat dalam logik main.go
		{"Safe HATA link", "Check https://hata.io", false}, // Dianggap selamat dalam logik main.go
		{"No links", "Hello world", false},
		{"Mixed safe text", "Visit official site worldcoin.org", false}, // Tidak mengandungi http/www/.ba.ba
		{"Link with safe domain but suspicious prefix", "http://worldcoin.org.bad.com", true}, // Dianggap mencurigakan
		{"Link with safe domain", "https://worldcoin.org/join/4RH0OTE", false}, // Dianggap selamat
		{"Link with hata domain", "https://hata.io/signup?ref=HDX8778", false}, // Dianggap selamat
	}
