...
func TestCreateInlineKeyboard(t *testing.T) {
	...
	for i, button := range keyboard.InlineKeyboard[0] {
		...
		if button.CallbackData == nil || *button.CallbackData != firstRowButtons[i].data {
			t.Errorf("First row button %d callback data: expected %q, got %q", i, firstRowButtons[i].data, *button.CallbackData)
		}
	}

	// Test tombol baris kedua (URL)
...
func TestMessageHandling(t *testing.T) {
	testCases := []struct {
		...
		{"Claim command", "/claim", ""}, // Akan memanggil sendDetailedGuide
		{"Wallet command", "/wallet", ""}, // Akan memanggil sendDetailedGuide
		{"Cashout command", "/cashout", ""}, // Akan memanggil sendDetailedGuide
...
func TestSecurityCheck(t *testing.T) {
	tests := []struct {
		...
		{"Safe HATA link", "Check hata.io", false},
